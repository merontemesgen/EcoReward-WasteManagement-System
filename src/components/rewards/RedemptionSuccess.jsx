...
const RedemptionSuccess = () => {
  ...
  const { type = "Airtime", amount = "$7", pointsUsed = 700 } = location.state || {};

    return (
    <div className="min-h-screen bg-amber-50 px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-gray-500 hover:text-gray-800 text-sm transition">
          <ArrowLeft className="w-4 h-4" /> Back
        </button>
        <div className="w-9 h-9 rounded-full bg-green-700 flex items-center justify-center text-white font-bold text-sm">OC</div>
      </div>
      ...
      <div className="max-w-sm mx-auto flex flex-col items-center text-center gap-6 mt-10">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900">Redemption Successful!</h2>
          <p className="text-gray-500 text-sm mt-1">Your airtime will be processed within minutes</p>
        </div>

        <div className="w-full bg-green-50 rounded-2xl p-4 border border-green-100">
          {[{ label: "Type:", value: type }, { label: "Amount:", value: amount }, { label: "Points Used:", value: pointsUsed }].map((row) => (
